...
<body>
  <div class="header">
    <div>
      <div class="title">Ticket #${ticket.number} — ${escHtml(panel.name)}</div>
      <div style="margin-top:6px;display:flex;gap:8px;flex-wrap:wrap">
        <span class="badge badge-priority">${ticket.priority.toUpperCase()}</span>
    ...
    <div style="font-size:.8rem;color:#72767d">Generated ${new Date().toUTCString()}</div>
  </div>

  <div class="info-grid">
    <div class="info-cell"><label>Opened by</label><span>${escHtml(ticket.username)}</span></div>
    <div class="info-cell"><label>Status</label><span>${ticket.status.charAt(0).toUpperCase() + ticket.status.slice(1)}</span></div>
    <div class="info-cell"><label>Panel</label><span>${escHtml(panel.name)}</span></div>
    <div class="info-cell"><label>Opened at</label><span>${new Date(ticket.createdAt).toLocaleString()}</span></div>
    ${ticket.closedAt ? `<div class="info-cell"><label>Closed at</label><span>${new Date(ticket.closedAt).toLocaleString()}</span></div>` : ''}
    ${ticket.claimedByTag ? `<div class="info-cell"><label>Claimed by</label><span>${escHtml(ticket.claimedByTag)}</span></div>` : ''}
    ${ticket.reason ? `<div class="info-cell"><label>Reason</label><span>${escHtml(ticket.reason)}</span></div>` : ''}
    ${ticket.tags.length > 0 ? `<div class="info-cell"><label>Tags</label><span>${ticket.tags.map(escHtml).join(', ')}</span></div>` : ''}
    ${ticket.rating ? `<div class="info-cell"><label>Rating</label><span>${'⭐'.repeat(ticket.rating)} (${ticket.rating}/5)</span></div>` : ''}
    ${ticket.ratingFeedback ? `<div class="info-cell" style="grid-column:span 2"><label>Rating Feedback</label><span>${escHtml(ticket.ratingFeedback)}</span></div>` : ''}
    <div class="info-cell"><label>Messages</label><span>${messages.length}</span></div>
  </div>

  <div class="messages">
    <div style="font-size:.8rem;color:#72767d;margin-bottom:12px">${messages.length} messages</div>
    ${rows}
  </div>
  ...
  <footer>Generated by the Tickets addon • ${new Date().toUTCString()}</footer>
</body>
</html>`;
