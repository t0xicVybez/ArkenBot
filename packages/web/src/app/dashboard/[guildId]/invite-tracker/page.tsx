    ...
    <div className="rounded-lg bg-white/[0.03] border border-white/[0.04] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {invitees.length > 0 && (
            <span className="text-[11px] text-gray-500">({invitees.length} member{invitees.length !== 1 ? 's' : ''})</span>
          )}
        </div>
        <div className="flex items-center gap-3 text-sm">
          <span className="text-gray-400">{row.invites} invite{row.invites !== 1 ? 's' : ''}</span>
          {row.bonus !== 0 && (
            <span className={row.bonus > 0 ? 'text-green-400' : 'text-red-400'}>
              {row.bonus > 0 ? '+' : ''}{row.bonus} bonus
            </span>
          )}
          <span className="text-discord-blurple font-semibold">{total} total</span>
          {invitees.length > 0
            ? expanded
      ...
      {expanded && invitees.length > 0 && (
        <div className="border-t border-white/[0.04] px-3 py-2 space-y-1.5 bg-black/20">
          <p className="text-[10px] uppercase tracking-widest text-gray-600 font-semibold mb-2">
            Invited Members — {stillHere} still here · {left} left
          </p>
          {invitees.map((invitee) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {invitee.leftAt && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-red-500/15 text-red-400 font-medium">left</span>
                )}
              </div>
              <span className="text-[11px] text-gray-600">
                Joined {new Date(invitee.joinedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
              </span>
            </div>
          ))}
        </div>
    ...
    <div className="p-3 sm:p-6 max-w-3xl">
      ...
      <SettingsSection title="Settings" description="Track how many members each user has invited to the server.">
        <label className="flex items-center justify-between">
          <span className="text-gray-300 text-sm">Enable Invite Tracker</span>
          <div
        ...
        {!enabled && (
          <p className="text-gray-500 text-xs">Enable invite tracking to start recording invite data. Uses your bot&apos;s existing invite permissions.</p>
        )}
      </SettingsSection>

      <SettingsSection
        title="Top Inviters Leaderboard"
        description="Click a row to see which members joined through that person's invite."
      >
        {leaderboard.length === 0 ? (
          <div className="text-center py-8">
            <Trophy className="w-10 h-10 text-gray-600 mx-auto mb-2" />
            <p className="text-gray-400 text-sm">No invite data yet.{!enabled && ' Enable invite tracking above to start.'}</p>
          </div>
        ) : (
