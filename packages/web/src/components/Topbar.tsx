...
export function Topbar({ variant = 'guild', guildName, guildId }: TopbarProps) {
  ...
  return (
    <div className="hidden md:flex sticky top-0 z-40 items-center gap-3.5 h-[54px] px-[26px] bg-[rgba(12,14,19,0.85)] backdrop-blur-[10px] border-b border-(--border-subtle)">
      <div className="text-[12.5px] text-(--text-muted) truncate">
        {variant === 'staff' ? (
          <span className="inline-flex items-center gap-2">
            <span className="text-[9.5px] font-extrabold tracking-widest px-2 py-0.5 rounded-full bg-(--accent-glow) text-(--accent) border border-(--accent)/30">{t('staffBadge')}</span>
            <b className="text-(--text-primary) font-semibold">{title}</b>
          </span>
        ) : (
          <>
      ...
      {variant === 'guild' && (
        <>
          <button
            ...
            title={t('searchTitle')}
            className="w-8 h-8 grid place-items-center rounded-lg text-(--text-secondary) hover:bg-white/6 transition-colors"
          >
      ...
      {user?.avatar ? (
        ...
        <div className="w-[30px] h-[30px] rounded-full bg-(--accent) grid place-items-center text-[11px] font-extrabold text-white">
