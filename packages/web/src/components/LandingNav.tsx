...
export function LandingNav({ docsUrl, supportUrl, inviteUrl }: LandingNavProps) {
  ...
  // The auth store hydrates from localStorage on the client, so gate on a mounted
  // flag to avoid a server/client markup mismatch (matches the dashboard pattern).
  ...
  return (
    <nav className={clsx(
      ...
      scrolled
        ? 'border-(--border) bg-(--bg-surface)/95 backdrop-blur-xl'
        : 'border-transparent bg-(--bg-surface)/60 backdrop-blur-xs',
    )}>
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 rounded-lg hover:opacity-80 transition-opacity" aria-label={t('homeAria')}>
          <div className="w-7 h-7 bg-discord-blurple rounded-lg flex items-center justify-center shadow-xs">
        ...
        <div className="hidden md:flex items-center gap-1 text-sm">
          ...
          <a
            ...
            className="px-3 py-1.5 rounded-lg text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/4 transition-colors flex items-center gap-1.5"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="px-3 py-1.5 rounded-lg text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/4 transition-colors"
          >
            {t('support')}
          </a>
        </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-lg text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/6 transition-colors"
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-(--border) bg-(--bg-elevated) backdrop-blur-md px-4 py-3 space-y-1">
          {NAV_LINKS.map((link) => {
            const isPageLink = !link.href.startsWith('#');
            if (isPageLink) {
              return (
                <Link
                  ...
                  className="block w-full text-left px-3 py-2.5 rounded-lg text-sm text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/5 transition-colors"
                >
                  {t(link.key)}
                </Link>
              );
            ...
            return (
              <button
                key={link.href}
                onClick={() => handleAnchorClick(link.href)}
                className="w-full text-left px-3 py-2.5 rounded-lg text-sm text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/5 transition-colors"
              >
                {t(link.key)}
              </button>
            );
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => setMobileOpen(false)}
            className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/5 transition-colors"
          >
            {t('docs')} <ExternalLink className="w-3 h-3" />
          </a>
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => setMobileOpen(false)}
            className="flex items-center px-3 py-2.5 rounded-lg text-sm text-(--text-secondary) hover:text-(--text-primary) hover:bg-white/5 transition-colors"
          >
            {t('support')}
          </a>
