...
export function SettingsSection({ title, description, children, danger = false }: SettingsSectionProps) {
  return (
    <div className={`card p-0 overflow-hidden mb-6 ${danger ? 'border-[rgba(242,109,95,0.28)]' : ''}`}>
      <div className={`px-[18px] py-[13px] border-b ${danger ? 'border-[rgba(242,109,95,0.18)]' : 'border-(--border-subtle)'}`}>
        <h3 className={`text-[13.5px] font-bold ${danger ? 'text-(--danger)' : 'text-white'}`}>{title}</h3>
